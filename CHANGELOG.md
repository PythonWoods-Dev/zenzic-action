...
- **Stale GitHub Org Slug (`PythonWoods/zenzic-action` → `PythonWoods-Dev/zenzic-action`)**: the org was renamed at some point, but `README.md`'s badges (CI-status, version-tag) and cross-repo links, `package.json`, `pyproject.toml`, `RELEASE.md`, `CONTRIBUTING.md`, `SECURITY.md`, the 3 issue templates, and `self-check.yml`'s real checkout/`ls-remote` steps against the `zenzic` Core sibling repo all still referenced the old org. Non-breaking today (GitHub 301-redirects; live-verified via `curl`/`gh api`), but non-canonical. Same defect class already fixed in `zenzic` Core's own files this session.
